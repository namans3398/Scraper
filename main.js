...
ipcMain.handle('get-video-info', async (event, url) => {
  ...
  if (!ytdlpAvailable) {
    throw { error: 'yt-dlp is not installed or not found in PATH. Please install it first.' };
  }
    ...
    ytdlp.on('close', (code) => {
      if (code === 0) {
        try {
          ...
          reject({ error: 'Failed to parse video information', details: e.message });
        }
      } else {
        reject({ error: 'Failed to fetch video information', details: stderr || 'Unknown error' });
      }
    });
...
// Download video
ipcMain.handle('download-video', async (event, { url, formatId, outputPath }) => {
